...
interface ServiceOrderFilesProps {
  orderId: string
  canUpload?: boolean
}

export function ServiceOrderFiles({ orderId, canUpload = true }: ServiceOrderFilesProps) {
  ...
  const fetchOrderFiles = async () => {
    try {
      const response = await serviceOrderFilesApi.getFiles(orderId)
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      const response = await serviceOrderFilesApi.uploadFiles(orderId, formData)
  ...
  const handleDeleteFile = async (fileId: string) => {
    ...
    try {
      await serviceOrderFilesApi.deleteFile(fileId)
  ...
  const handleDownloadFile = async (file: ServiceOrderFile) => {
    try {
      const response = await serviceOrderFilesApi.downloadFile(file.id)
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-lg shadow-sm p-6">
        <h3 className="font-semibold text-gray-900 mb-4">Order Files</h3>
    ...
    <>
      <div className="bg-white rounded-lg shadow-sm p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-gray-900">Order Files</h3>
          {canUpload && (
            <Button
