...
export default function ProjectTimeline({ projectId }: ProjectTimelineProps) {
  ...
  const renderEventDetails = (event: ProjectEvent) => {
    ...
    switch (event.eventType) {
      case 'FREELANCER_HIRED':
        return (
          <div className="text-sm text-gray-600 mt-1">
            <p><span className="font-medium">{metadata.freelancerName}</span> hired for ${metadata.agreedAmount}</p>
            <p className="text-xs text-gray-500">Total charged: ${metadata.totalCharged} (includes fees)</p>
          </div>
        )
      ...
      case 'ESCROW_FUNDED':
        return (
          <div className="text-sm text-gray-600 mt-1">
            ...
            <p className="text-xs text-gray-500">Transaction ID: {metadata.stripePaymentId?.slice(-8)}</p>
          </div>
        )
      ...
      case 'REVISION_SUBMITTED':
        return (
          <div className="text-sm text-gray-600 mt-1">
            <p>{metadata.message || 'Work ready for review'}</p>
          </div>
        )
      ...
      case 'PROJECT_APPROVED':
        return (
          <div className="text-sm text-gray-600 mt-1">
            <p>Project work approved by client</p>
          </div>
        )
      ...
      case 'PROJECT_CANCELLED':
        return (
          <div className="text-sm text-gray-600 mt-1">
            <p>{metadata.reason || 'Project cancelled'}</p>
          </div>
        )
